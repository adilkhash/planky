...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("llm_service.log"), logging.StreamHandler()],
)
...
class LLMService:
    ...
    def _prepare_tag_generation_prompt(
        self, content: str, max_tags: int = 5, tag_format: str = "comma"
    ) -> tuple:
        """
    ...
    def _extract_tags_from_response(
        self, response: dict, tag_format: str = "comma"
    ) -> list[str]:
        """
        ...
        try:
            ...
            if tag_format == "comma":
                ...
                try:
                    ...
                    logger.warning(
                        "Failed to parse JSON tags, falling back to comma-separated"
                    )
                    tags = [tag.strip() for tag in content.split(",")]
            elif tag_format == "list":
    ...
    def generate_tags(
        ...
        try:
            ...
            system_message, user_message = self._prepare_tag_generation_prompt(
                content=content, max_tags=max_tags, tag_format=tag_format
            )
